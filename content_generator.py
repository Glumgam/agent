...
# 後処理: 必須セクション自動補完
# ---------------------------------------------------------------------------
def _ensure_required_sections(content: str, finance_data: dict) -> str:
    """
    「明日の戦略」セクションが欠如している場合に市場データから自動補完する。
    LLMに再生成させず、実データから直接テキストを生成する。
    """
    if "明日の戦略" in content:
        return content  # 既に存在すればスキップ（📈 付き見出しも "明日の戦略" を含む）

    macro    = finance_data.get("macro", {})
    us       = macro.get("us_stocks", {})
    forex    = macro.get("forex", {})
    vix_val  = us.get("VIX", {}).get("price") or 20.0
    vix_chg  = us.get("VIX", {}).get("change_pct") or 0.0
    usdjpy_chg = forex.get("USD/JPY", {}).get("change_pct") or 0.0

    if isinstance(vix_val, (int, float)) and vix_val < 20 and isinstance(vix_chg, (int, float)) and vix_chg < 0:
        scenario = "リスクオン継続の可能性が高い局面です。ただし過熱感が出た銘柄は利確に注意が必要です。"
    elif isinstance(vix_val, (int, float)) and (vix_val >= 25 or (isinstance(vix_chg, (int, float)) and vix_chg > 5)):
        scenario = "市場の警戒感が強く、慎重な姿勢が求められる局面です。"
    else:
        scenario = "方向感が定まりにくい局面で、上値も下値も重い展開が意識されやすいです。"

    if isinstance(usdjpy_chg, (int, float)) and usdjpy_chg > 0.5:
        fx_note = "円安が継続しており、輸出関連を中心に下値は堅い展開が続きやすいです。"
    elif isinstance(usdjpy_chg, (int, float)) and usdjpy_chg < -0.5:
        fx_note = "円高進行に注意が必要で、輸出関連セクターへの影響が意識されやすいです。"
    else:
        fx_note = "為替は小幅な動きで、輸出入両セクターへの影響は限定的と見られます。"

    strategy_section = (
        "\n## 📈 明日の戦略\n"
        f"- **基本シナリオ**: {scenario}\n"
        "- **狙い**: 高配当・PBR割安株が相対的に強い局面が続いています。"
        "ディフェンシブセクターに注目が集まりやすいです。\n"
        "- **回避**: 材料なし急騰銘柄・高PERグロース株は特に注意が必要です。\n"
        "- **危険シナリオ**: VIX急反発時・円高転換時の影響が意識されやすいです。\n"
        f"- **補足**: {fx_note}\n"
    )

    # 挿入位置：注目テーマ → 今後の注目点 → まとめ の順に優先
    for anchor in ("## 🎯 注目テーマ", "## 今後の注目点", "## まとめ", "## 📊 まとめ"):
        if anchor in content:
            content = content.replace(anchor, strategy_section + "\n" + anchor, 1)
            print("  🔧 「明日の戦略」セクションを自動補完しました")
            return content

    # フォールバック：末尾に追加
    content += "\n" + strategy_section
    print("  🔧 「明日の戦略」セクションを末尾に自動補完しました")
    return content
...
# 後処理: 普通体 → ですます体 強制変換
# ---------------------------------------------------------------------------
def _enforce_desu_masu(content: str) -> str:
    """
    文末の普通体をですます体に強制変換する。
    見出し・URL・HTML・コードブロック行はスキップする。
    """
    _SKIP = re.compile(
        r'^(?:#|http|\s*[-*]|\s*\d+\.|\s*[<|]|```|\s*$)'
    )

    # (pattern, replacement) — 文末（行末）のみ対象
    _REPLACEMENTS = [
        (r'した。$',           'しました。'),
        (r'である。$',         'です。'),
        (r'している。$',       'しています。'),
    ...
    for line in content.split('\n'):
        if line.strip().startswith('```'):
            ...
            lines.append(line)
            continue
        if in_code or _SKIP.match(line):
            lines.append(line)
            continue
...
_IRRELEVANT_KWS = [
    "逮捕", "わいせつ", "殺人", "殺害", "遺体", "強盗",
    "痴漢", "暴行", "詐欺師", "芸能", "スポーツ選手",
    "ストーカー", "監禁", "天井裏に侵入", "准教授が",
    "警察官が酒気帯び", "大麻", "不起訴",
]

# 法務セクション内でも保持すべき企業・市場関連キーワード
_CORPORATE_KWS = [
    "金融庁", "公正取引委員会", "課徴金", "行政処分",
    "上場企業", "株式", "証券", "企業", "会社", "市場",
    "取引", "インサイダー", "有価証券",
]


def _filter_irrelevant_content(content: str) -> str:
    """
    投資・市場と無関係な記述を段落単位で除去する。
    【変更点】法務セクション内でも個人犯罪・社会事件は除去する。
    ただし企業・市場関連キーワードを含む段落は常に保持する。
    """
    ...
    for para in paragraphs:
        ...
        # 見出し行は常に保持（セクション追跡は廃止）
        if stripped.startswith('#'):
            filtered.append(para)
            continue

        # 企業・市場関連キーワードがあれば内容問わず保持
        if any(kw in para for kw in _CORPORATE_KWS):
            filtered.append(para)
            continue

        # 無関係キーワード検出 → 除去（法務セクション内も含む）
        if any(kw in para for kw in _IRRELEVANT_KWS):
            print(f"  🗑️ 無関係コンテンツを除去: {stripped[:50]}...")
            continue
...
- 銘柄名（変動率）
  ...
  - 背景: [コンテキストの【銘柄背景推定（AI分析）】に当該銘柄の説明があれば必ずそれを使用。ない場合は本日のセクター動向・マクロ環境を根拠にした背景推定を記述すること]
値下がり上位:（同様のフォーマット）
【禁止事項】
- 「〜の期待が背景」「〜の可能性がある」等の推測表現は禁止
- コンテキストにない具体的な決算・提携情報を捏造することは禁止
- 「背景は未公表」は直接的な材料が全くない場合のみ使用可（最大2回まで）
## 本日の市場のポイント（3つ）
...
【文体ルール（最重要）】
- 全文をですます体（〜です・〜ます・〜ました・〜ています）に統一する
- 「〜である」「〜した」「〜している」「〜終えた」「〜なった」等の普通体は絶対に使わない
- 文末は必ず「〜です」「〜ます」「〜ました」「〜ています」で終えること
...
【高度な分析ロジック（必ず適用）】
■ 米国株上昇なのに日経下落の場合:
→ 「リスクオンの中でのポジション調整」と解釈する
→ 「利確・セクターローテーションが主因と見られます」と記述する
→ 単純に「外部環境悪化」と書かない
■ 原油急落（-5%以上）の解釈:
→ 「良い下げ」か「需要減速懸念型の下げ」かを判断する
→ VIXが低下中なら「エネルギーコスト低下メリット型」
→ VIXが上昇中なら「景気減速シグナル型」
→ どちらかを必ず明記する
■ 円安の質を判断する:
→ 急激な円安（1日で0.5%以上）かつ日経下落 →「逃げの円安（日本売り）の可能性があります」と記述
→ 緩やかな円安かつ日経上昇 →「輸出企業に恩恵をもたらす良い円安と見られます」と記述
■ 米国株上昇＋VIX低下＋日経下落 の組み合わせ:
→ 「過熱調整局面」と判断する
→ 「暴落の始まりではなく、短期的なポジション整理と見られます」と記述
■ 原油上昇（+5%以上）の質を判断する:
→ ニュースに「地政学・中東・イラン・OPEC・供給不安」が含まれる場合:
  「地政学リスク主導の原油高。景気回復による需要増ではなく供給不安が要因のため、
   航空・運輸・内需消費セクターには逆風となる可能性があります」と記述する
→ ニュースに「景気回復・需要増・経済成長」が含まれる場合:
  「需要増を背景とした原油高。資源・商社セクターに恩恵が及ぶ可能性があります」と記述する
■ ねじれ相場（米株↑・VIX↓・原油急騰）の場合:
→「表面上リスクオンだが、地政学リスクが潜在的な不安要因として残っています」と記述する
→ 明日の戦略の基本シナリオは「上値の重いレンジ相場」とする
→「上昇しても継続性は限定的と見られます」と付記する
■ セクター別の原油影響（原油±5%以上の場合に必ず記述）:
→ 原油上昇時:
  恩恵: 「資源・エネルギー株・商社」
  逆風: 「航空・陸運・内需消費」
→ 原油下落時:
  恩恵: 「航空・運輸・化学・製造業全般」
  逆風: 「資源・エネルギー株」
■ VIX上昇（前日比+1%以上）かつ米株下落 → 日経上昇の場合:
→ 「リスクオン継続」と書かない
→ 「日本株が相対的な逃避先として買われている局面」と記述する
→ 「外部環境の悪化が続けば連れ安のリスクがあります」と付記する
→ 明日の戦略の基本シナリオは「上値の重いレンジ」とする
■ 金価格上昇（+0.5%以上）かつVIX上昇の場合:
→ 「安全資産への資金流入が見られ、リスクオフの色合いが強い局面です」と記述する
→ 「表面上の株価上昇に惑わされず、慎重な姿勢が望ましい局面と見られます」と付記する）
## 🌏 マクロ経済環境
...
- 銘柄名（変動率）
  - 関連ニュース: [コンテキストの「値動き銘柄の関連ニュース」に記載がある場合のみ。なければ「関連ニュースなし」]
  - 背景: [コンテキスト先頭の【AI銘柄背景推定】に当該銘柄の記載があれば必ずそれを使用。なければ本日のセクター動向・マクロ環境（円安/円高・VIX・米株）を根拠に銘柄固有の背景を記述]
値下がり上位:（同様のフォーマット）
【重要ルール（違反すると記事が不合格になる）】
- 全銘柄で同じ文章を使い回すことは絶対禁止（各銘柄に固有の記述が必要）
- 「短期資金による需給主導の動き」を複数銘柄に使い回すことは禁止
- 「背景は未公表」「未公表のため」は使用禁止
- コンテキストにない具体的な決算・提携情報を捏造することは禁止
【背景記述の多様な例（各銘柄で異なる表現を使うこと）】
- 〇〇セクターへの資金流入が継続し、当銘柄にも買いが波及しました。
- 本日の円安進行を受け、輸出採算の改善期待から買いが集まりました。
- 市場全体のリスクオフの流れを受け、値動きが出やすい小型株として売りが優勢でした。
- 同セクター他社の業績改善報道を受けた連れ高の動きと見られます。
- 本日のVIX低下を背景にグロース系小型株への物色が広がりました。
## ⚖️ 法務・規制動向
...
def _fix_typos(content: str) -> str:
    """文法ミス・誤字を機械的に修正する"""
...
def generate_article(
    ...
    if genre_id == "finance_news" and not extra_context:
        try:
            ...
            # AIによるニュースフィルタリングと銘柄背景推定
            try:
                from llm import filter_investment_news, analyze_stock_background
                all_news = _data.get("news", [])
                if all_news:
                    print(f"  🤖 AIニュース選択中（{len(all_news)}件 → 5件）...")
                    _data["news_filtered"] = filter_investment_news(all_news, max_count=5)
                    print(f"  ✅ 選択完了: {len(_data['news_filtered'])}件")
                else:
                    _data["news_filtered"] = []

                up_stocks        = _data.get("up_ranking", [])[:3]
                down_stocks      = _data.get("down_ranking", [])[:3]
                all_target_stocks = up_stocks + down_stocks
                if all_target_stocks:
                    print(f"  🤖 銘柄背景推定中（{len(all_target_stocks)}銘柄）...")
                    _data["stock_backgrounds"] = analyze_stock_background(
                        all_target_stocks, _data
                    )
                    print(f"  ✅ 背景推定完了: {len(_data['stock_backgrounds'])}銘柄")
                else:
                ...
                _data["news_filtered"]     = _data.get("news", [])[:5]
                _data["stock_backgrounds"] = {}
    ...
    # 投資記事: AI銘柄背景推定をコンテキスト先頭に強調挿入
    # （LLMはプロンプト前半を重視するため先頭に配置する）
    if genre_id == "finance_news" and _finance_data_for_check:
        bgs = _finance_data_for_check.get("stock_backgrounds", {})
        if bgs:
            bg_lines = "\n".join(f"  {name}: {bg}" for name, bg in bgs.items())
            ai_bg_block = (
                "【AI銘柄背景推定（重要: 値動きランキング解説で必ず反映すること）】\n"
                + bg_lines
                + "\n【注意】全銘柄で異なる背景文を書くこと。コピペ・使い回し禁止。\n"
            )
            context = ai_bg_block + "\n\n" + context
    ...
    if is_finance and _finance_data_for_check:
        try:
            _nikkei_chg_raw = _finance_data_for_check.get("market_summary", {}).get("nikkei_change", "") or ""
            _m = re.search(r'([+-]?\d+\.?\d*)\s*%', _nikkei_chg_raw)
            _nikkei_chg = float(_m.group(1)) if _m else None
            if _nikkei_chg is not None:
                if _nikkei_chg >= 0:
                    content = re.sub(
                        r'##\s*📊\s*この(?:上昇|下落)は続くのか？',
                        '## 📊 この上昇は続くのか？', content
                    )
                else:
                    content = re.sub(
