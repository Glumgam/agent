...
## 値動きのポイント
（コンテキストの[重要:最優先]セクションにある銘柄名と変動率のみ記載する。
変動率以外の情報はコンテキストの「値動き銘柄の関連ニュース」に根拠がある場合のみ記載。
根拠がない場合は「本日の変動率: +XX%（背景は未公表）」とだけ書く。
「業績改善の期待」「新規プロジェクト」等の根拠のない推測を書かない。）
## 本日の市場のポイント（3つ）
...
## 値上がり・値下がりランキング解説
【銘柄説明のルール（厳守）】
- コンテキストの「値動き銘柄の関連ニュース」に根拠がある場合のみ理由を記載
- 根拠がない場合は「変動率: +XX%（背景は未公表）」とだけ書く
- 「業績改善の期待」「新規プロジェクト」等の根拠のない推測は禁止
- 「具体的な背景は公表されていない」はOK（正直な表現）
## ⚖️ 法務・規制動向
