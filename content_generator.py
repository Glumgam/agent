...
def _filter_irrelevant_content(content: str) -> str:
    """
    投資と無関係な社会ニュースの段落を除去する。
    法務・規制動向セクション内の段落は除外しない。
    """
    ...
    for para in paragraphs:
        ...
        if stripped.startswith('#'):
            in_legal = any(kw in stripped for kw in ("法務", "規制", "行政処分"))
        ...
        if any(kw in para for kw in _IRRELEVANT_KWS):
            print(f"  🗑️ 無関係コンテンツを除去: {stripped[:40]}...")
            continue
        filtered.append(para)
...
- 銘柄名（変動率）
  - 関連ニュース: [コンテキストの「値動き銘柄の関連ニュース」に記載がある場合のみ。なければ「関連ニュースなし」]
  - 背景: [コンテキストの【銘柄背景推定（AI分析）】に当該銘柄の説明があれば必ずそれを使用。ない場合は本日のセクター動向・マクロ環境（円安/円高・VIX・米株）を根拠にした背景推定を記述すること]
値下がり上位:（同様のフォーマット）
【禁止事項】
- 「〜の期待が背景」「〜の可能性がある」等の推測表現は禁止
- コンテキストにない具体的な決算・提携情報を捏造することは禁止
- 「背景は未公表」は直接的な材料が全くない場合のみ使用可（最大2回まで）
【背景記述の例】
- 本日の円安進行（USD/JPY +X.X円）を受けた輸出関連セクターへの買いが波及しました。
- リスクオフの流れからディフェンシブ株への資金シフトが進みました。
- 短期資金による需給主導の動きで、特定の材料は確認されていません。
## ⚖️ 法務・規制動向
