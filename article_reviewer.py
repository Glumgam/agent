...
1. 文体統一: ですます体が全文で統一されているか（「した。」「である。」との混在でマイナス）
2. 数値正確性: 日経平均・VIX・為替の方向性・数値に明らかな誤りがないか
3. 構成完整性: 必須セクション（市場概況・明日の戦略・注目テーマ・この上昇は続くのか・まとめ・免責事項）が全て含まれているか
4. 日本語品質: 自然な日本語か、英語混入・中国語混入・文法誤りがないか
5. 情報密度: 2000文字以上で具体的な数値・銘柄情報が含まれているか

必ず以下のフォーマットのみで回答:
SCORE: [5項目の合計を0-10の整数で]
